...
Εξοδος: player_career.jsonl — {pid, s: [[seasonName, leagueId, leagueName, apps, rating, isFriendly], ...]}
"""
...
OUT = 'player_career.jsonl'
targets = []
for line in open('player_backfill.jsonl', encoding='utf-8'):
    r = json.loads(line)
    rated = [m for m in r['m'] if m[4] is not None]
    if len(rated) < 10:
        targets.append((r['pid'], r['nm']))
done = set()
...
print(f'ΤΕΛΟΣ career: ok {ok} · err {err} · {(time.time()-t0)/60:.0f} λεπτα', flush=True)
