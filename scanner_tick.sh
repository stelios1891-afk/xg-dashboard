...
# 26/9: ημερολογιο ευρωπαϊκων picks για το Pick History (εισοδος ≤72ω, κλεισιμο, σκορ/xG FotMob)
python euro_picks_ledger.py || echo "euro ledger failed (μη κρισιμο)"
...
# 25/9: κανονικα picks εθνικων (συναινεση ≥2/3) — πρωτη εμφανιση ≤72ω + εκκαθαριση (intl_picks_ledger.jsonl)
python intl_picks_ledger.py || echo "intl ledger failed (μη κρισιμο)"
# 26/9: επισημες αποστολες FotMob ~60′ πριν → μετατοπιση τιμων Μ1/Μ3 + Telegram (intl_lineups.json)
...
python el_picks.py || echo "el picks failed (μη κρισιμο)"   # 25/9: value picks Ευρωλιγκας + Telegram
python el_report.py weekly || echo "el report failed (μη κρισιμο)"   # 26/9: εβδομαδιαια αναφορα EL (Δευτερα) → Telegram

# ---- commit ΜΟΝΟ οταν αλλαξαν picks/market (ιδιο gate με πριν) ----
git config user.name "scanner-bot"
git config user.email "actions@github.com"
for f in clv_bets.jsonl clv_ledger.jsonl shadow_picks.jsonl shadow_state.json brazil_shadow.jsonl brazil_shadow_state.json euro_odds_latest.json dom_odds_latest.json euro_shadow.jsonl euro_shadow_state.json euro_value_latest.json euro_live_odds.jsonl euro_odds_hist.jsonl dom_odds_hist.jsonl dom_live_odds.jsonl dom_fav_shadow.jsonl dom_fav_shadow_state.json dom_uncomp_pairs.json intl_odds_latest.json intl_odds_hist.jsonl intl_live_odds.jsonl intl_closing.jsonl intl_picks_ledger.jsonl euro_picks_ledger.jsonl intl_lineups.json intl_refresh_due_state.json intl_projections_dashboard.json el_odds_latest.json el_odds_hist.jsonl el_value_latest.json el_value_state.json el_clv_bets.jsonl; do [ -f "$f" ] || : > "$f"; done
git add -f value_last_scan.txt el_report_last.txt 2>/dev/null || true   # ποτε εγινε το τελευταιο TOA scan — χωρις αυτο καθε runner ξαναπληρωνε (24/9)
git add -f intl_odds_latest.json intl_odds_hist.jsonl intl_live_odds.jsonl intl_closing.jsonl intl_picks_ledger.jsonl euro_picks_ledger.jsonl intl_lineups.json intl_refresh_due_state.json intl_projections_dashboard.json 2>/dev/null || true   # 25/9 intl (δεν ειναι gitignored· -f για σιγουρια)
git add value_scan_state.json value_picks_latest.json market_1x2_latest.json odds_history.jsonl odds_history_state.json clv_bets.jsonl clv_ledger.jsonl shadow_picks.jsonl shadow_state.json brazil_shadow.jsonl brazil_shadow_state.json euro_odds_latest.json dom_odds_latest.json euro_shadow.jsonl euro_shadow_state.json euro_value_latest.json euro_live_odds.jsonl euro_odds_hist.jsonl dom_odds_hist.jsonl dom_live_odds.jsonl dom_fav_shadow.jsonl dom_fav_shadow_state.json dom_uncomp_pairs.json el_odds_latest.json el_odds_hist.jsonl el_value_latest.json el_value_state.json el_clv_bets.jsonl
if git diff --cached --quiet -- value_last_scan.txt value_scan_state.json market_1x2_latest.json odds_history_state.json clv_bets.jsonl clv_ledger.jsonl shadow_picks.jsonl brazil_shadow.jsonl euro_odds_latest.json dom_odds_latest.json euro_shadow.jsonl euro_value_latest.json euro_live_odds.jsonl euro_odds_hist.jsonl dom_odds_hist.jsonl dom_live_odds.jsonl dom_fav_shadow.jsonl dom_fav_shadow_state.json dom_uncomp_pairs.json intl_odds_latest.json intl_odds_hist.jsonl intl_live_odds.jsonl intl_closing.jsonl intl_picks_ledger.jsonl euro_picks_ledger.jsonl intl_lineups.json intl_refresh_due_state.json intl_projections_dashboard.json el_odds_latest.json el_odds_hist.jsonl el_value_latest.json el_value_state.json el_clv_bets.jsonl el_report_last.txt; then
