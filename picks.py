...
SOS = 1.5; SOS_MIN_N = 6; SOS_MAX_N = 13   # Strength-of-Schedule (Caley one-pass). 1.5 (2026-08-27, sos_clean_loso.py/blend_ramp.py):
            ...
            # τα ratings· απο τη 15η τα προγραμματα εχουν εξισωθει". Τα δεδομενα συμφωνουν στην
            # ΑΚΡΙΒΕΙΑ: RPS@md15+ 0.1967 (OFF) -> 0.1977 (1.5), μονοτονη επιδεινωση.
            # Κοστος της επιλογης, ρητα: md15+ 80.9u -> 60.9u και 4/4 -> 3/4 σεζον (τα 550 extra
            # στοιχηματα που ανοιγε το SoS εβγαζαν +3.94%). Ολες οι διαφορες μεσα σε 1 SE (sos15_final.py).
...
def sos_adjust(r, opps, ratings, lg_shots, lg_xgps, strength=SOS):
    ...
    Ενεργο ΜΟΝΟ για n=SOS_MIN_N..SOS_MAX_N αντιπαλους (αγωνιστικες 7-14)."""
    if not opps or not strength or not (SOS_MIN_N <= len(opps) <= SOS_MAX_N):
        return r
    oA = []; oD = []; oSF = []; oSA = []
    for o in opps:
        a = ratings.get(o)
