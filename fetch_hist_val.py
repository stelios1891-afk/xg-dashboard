"""Κατεβαζει FotMob shot-data για LaLiga+Primeira ιστορικες σεζον (2122/2223/2324)
για OOS validation του value-over. Ιδια μορφη με data_<name>_<code>.json. Resume-safe."""
...
JOBS=[('LaLiga',87,[('2021%2F2022','2122'),('2022%2F2023','2223'),('2023%2F2024','2324')]),
      ('PrimeiraLiga',61,[('2021%2F2022','2122'),('2022%2F2023','2223'),('2023%2F2024','2324')])]
for name,lid,seasons in JOBS:
    for season,code in seasons:
        ...
        for i,mid in enumerate(todo):
            ...
            if (i+1)%25==0:
                json.dump(done,open(path,'w',encoding='utf-8')); print(f"   {name} {code}: {i+1}/{len(todo)}",flush=True)
            time.sleep(0.15)
