"""Turkish + Danish domestic shot-level (25/26) απο FotMob, ιδια μορφη με dl.py."""
...
def parse(mid):
    ...
    for e in c.get('matchFacts',{}).get('events',{}).get('events',[]) or []:
        if e.get('type')=='Card' and e.get('card') in ('Red','RedYellow'): reds.append({'home':bool(e.get('isHome')),'min':e.get('time')})
...
JOBS=[('TurkishSuperLig',71),('DanishSuperLiga',46)]
for name,lid in JOBS:
