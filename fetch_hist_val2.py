"""Ιστορικα FotMob shots για EPL/SerieA/Bundesliga/Ligue1 (2122/2223/2324) — full πριν/μετα."""
...
JOBS=[('EPL',47,[]),('SerieA',55,[]),('Bundesliga',54,[]),('Ligue1',53,[])]
seas=[('2021%2F2022','2122'),('2022%2F2023','2223'),('2023%2F2024','2324')]
JOBS=[(n,i,seas) for n,i,_ in JOBS]
for name,lid,seasons in JOBS:
    for season,code in seasons:
        ...
        ids=finished_ids(lid,season); todo=[m for m in ids if str(m) not in done]
        ...
        for i,mid in enumerate(todo):
            ...
            if (i+1)%25==0: json.dump(done,open(path,'w',encoding='utf-8')); print(f"   {name} {code}: {i+1}/{len(todo)}",flush=True)
            time.sleep(0.15)
        json.dump(done,open(path,'w',encoding='utf-8'))
        print(f"[{name} {code}] DONE {len(done)} ματς",flush=True)
